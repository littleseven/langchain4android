...
PicMe is an intelligent camera Android app with a self-developed real-time beauty/filter/makeup engine ("BIG_BEAUTY"). The project emphasizes AI-assisted development workflows, commercial-grade privacy/performance, and long-term evolution of the beauty engine into a standalone visual-capability library.
...
features/  →  domain/usecase/  →  domain/repository/  →  data/
   ↓                ↓
beauty-engine:api/  (strict boundary — see below)
```

- **Features**: Compose UI + ViewModels. Business logic lives in Repositories; UI state in ViewModels; rendering in Compose Screens.
- **Domain**: Pure Kotlin, no Android dependencies. Use cases and repository interfaces.
- **Data**: Repository implementations, Room DB, DataStore preferences, remote data sources.
