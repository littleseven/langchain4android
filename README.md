...
<h1 align="center">觅影相册-PicMe</h1>

<p align="center">
  <b>下一代端侧 AI 智能相册</b><br>
  <i>自然语言驱动 · 全链路 GPU 渲染 · 端侧智能</i>
</p>

<p align="center">
  <b>产品重心已迁移：从相机到相册与图片编辑</b><br>
  <i>本地模型 → 精简自定义协议 · 远程模型 → 标准 OpenAI API 协议</i>
</p>
...
PicMe 是一款探索「端侧 AI Agent 驱动应用」的智能相册应用。用户可以通过**自然语言**与照片交互——说「调高美颜」「帮我把天空调蓝」「找出去年夏天的照片」即可控制相册浏览、图片编辑和相机功能，无需手动调节复杂参数。

**2026-06 架构升级**：本地/远程推理协议正式分离。本地模型（Qwen3.5-2B）使用精简自定义 JSON 数组协议；远程模型使用标准 OpenAI Chat Completions API 协议（含 tool_calls、流式、多轮对话），充分发挥云端模型能力。

项目同时是一个**Agent First 工程试验场**，验证 Agent 作为研发流程第一公民的可行性，探索面向 Agent 的架构设计、协作机制与研发范式。

> 智能相册是试验载体，真正的研究对象是**端侧 Agent 运行时架构**与**Agent First 工程范式**。
...
| 你说 | PicMe 做 |
|------|----------|
| 「帮我把天空调蓝」 | 智能识别天空区域，调整色调 |
| 「找出去年夏天的照片」 | 相册智能搜索，按时间/内容精准匹配 |
| 「这张照片磨皮 50」 | 相册编辑页应用美颜参数 |
| 「把这些照片都调成复古风」 | 批量应用风格滤镜 |

- **端侧运行 Qwen3.5-2B 大模型**，本地推理处理相册/编辑指令，响应快、无网络依赖
- **远程模型采用标准 OpenAI Chat Completions API 协议**（配置个人 API Key），支持 tool_calls、流式输出、多轮对话，充分发挥云端模型能力
...
| 美颜渲染 | 本地 GPU | 零云端推理 |
| 远程推理（可选） | OpenAI 标准 API | 非敏感指令云端处理 |

PicMe 核心 AI 能力（美颜、人脸检测、OCR、本地指令解析）均可离线运行；远程 LLM 为可选增强，使用标准 OpenAI Chat Completions 协议。
...
│  │  │ Qwen3.5-2B       │  │ OpenAI 标准协议   │
│  │  │ MNN-LLM 推理     │  │ Chat Completions  │
│  │  │ 自定义 JSON 协议   │  │ tool_calls · 流式  │
│  │  │ (100% 离线)      │  │ (仅 PUBLIC 指令)  │
│  │  └────────┬─────────┘  └────────┬─────────┘              │
│  │           │                      │                        │
│  │  LocalInferencePipeline   RemoteInferencePipeline         │
│  │  PrivacyGuard · MemoryManager · SceneManager              │
│  │  CapabilityRegistry                                       │
...
AgentOrchestrator.dispatch()
    ├── 本地模式（LOCAL）：LocalInferencePipeline
    │   ├── LocalLlmEngine → MNN-LLM（Qwen3.5-2B）
    │   ├── 协议：自定义 JSON 数组（精简、高效）
    │   ├── GBNF Grammar 约束格式
    │   └── 端侧推理，无网络依赖，适合相册/编辑控制指令
    ├── 远程模式（REMOTE）：RemoteInferencePipeline
    │   ├── RemoteOrchestrator → OpenAI 标准 API
    │   ├── 协议：Chat Completions（含 tool_calls·流式·多轮对话）
    │   ├── LangChain4j SDK 序列化 Tool Schema
    │   └── 云端 LLM 编排，处理复杂推理与开放式对话
    └── 隐私守卫：PrivacyGuard.classify() 自动分级输入内容
        ├── RESTRICTED（坐标/人脸数据）→ 强制本地
        ├── SENSITIVE（照片/OCR）→ 强制本地
        └── PUBLIC（相册/编辑控制）→ 本地优先，可配置远程
    ↓
...
**关键设计决策**：
- **协议分离**（ADR-005）：本地模型用自定义 JSON 数组协议（method + args 平铺），远程模型用标准 OpenAI Chat Completions API（tool_calls·流式·多轮对话），LangChain4j SDK 仅作为远程端接入层
- **两条独立推理链路**：`LocalInferencePipeline` / `RemoteInferencePipeline`，无共享路由逻辑，各自独立演进
- **CapabilityRegistry 跨页面指令队列**：NavigationCapability 支持 "去相册给这张照片磨皮" 等跨场景指令，命令自动排队并在目标页面激活后执行
- **SceneManager 场景状态机**：Gallery / Editor / Camera / Settings 多场景，Capability 按场景注册和激活
...
| 维度 | 目标 | 当前状态 | 关键产出 |
|------|------|----------|----------|
| **端侧 Agent 运行时** | 验证 LLM 能否成为应用中枢神经系统 | ✅ 稳定运行 | Qwen3.5-2B 本地推理、15+ Capability 热插拔系统、多轮对话记忆、语音交互 |
...
| 5 | 端侧 LLM 满足相册/编辑控制推理 | ✅ — Qwen3.5-2B 本地推理 < 500ms，通过自定义简洁 JSON 指令格式（method + params 平铺结构）实现可靠的命令解析与指令翻译；OpenAI tool_calls 和 LangChain ToolSpec 多层嵌套结构对移动端小模型过于臃肿 |
...
端侧小模型（Qwen3.5-2B）对 OpenAI tool_calls 格式的 `function` 嵌套结构支持不稳定，超出其推理能力边界。实际采用 **自定义简洁 JSON 指令格式**（`method` + `params` 平铺结构）后，小模型指令生成的可靠性显著提升。LangChain 的 ToolSpec 机制虽然在大模型场景表现良好，但其多层嵌套结构对移动端小模型过于臃肿。
