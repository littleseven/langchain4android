...
| ADR | 决策 | 状态 | 说明 |
|-----|------|------|------|
| [ADR-001](docs/ADR-001-beauty-engine-architecture.md) | 大美丽与 GPUPixel 分层架构 | ✅ 已接受 | 完成 `beauty-engine:api` / `egl` / `gpupixel` 三层拆分，App 仅依赖 api 契约 |
| [ADR-002](docs/ADR-002-opengl-offscreen-unified-pipeline.md) | OpenGL 离屏渲染统一管线 | ✅ 已接受 | `OffscreenRenderer` 已落地于 `beauty-engine/impl`，预览/拍照使用同一套 Shader |
...
- `beauty-engine:egl/` 和 `beauty-engine:gpupixel/` 为 internal，不对外暴露
...
- **双引擎收敛**：当前仅保留自研大美丽 + GPUPixel 两条链路，所有旧兜底引擎实现与文档引用已清理
