...
- **端侧运行 Qwen3-1.7B 大模型**，100% 本地推理，隐私敏感数据零上传
- **远程 LLM 编排**（可选，高频迭代中）：配置个人 OpenAI/Claude 兼容 Token 解锁云端推理；腾讯云赠送额度已验证可用
...
- **文字交互**（相对稳定）：自然语言控制相机、调节参数、切换滤镜
- **语音控制 / 声控**（试验性探索中）：唤醒词触发、语音指令识别，仍在快速迭代验证阶段
...
- **双引擎人脸检测**：MediaPipe 468→106（主）+ InsightFace 2D106（备），支持 NCNN/MNN 多后端推理
...
│  └─ 相册浏览与静态图编辑                                       │
├──────────────────────────────────────────────────────────────┤
│  Agent Runtime (domain/agent/) —— 端侧 Agent 中枢神经系统      │
│  ├─ AgentOrchestrator      意图解析与任务编排                  │
│  ├─ InferenceRouter        本地/远程推理路由（自适应策略）      │
│  ├─ LocalLlmEngine         Qwen3-1.7B / MNN-LLM 端侧推理      │
...
│  └─ FaceDetectionEngine    MediaPipe + InsightFace 双引擎      │
└──────────────────────────────────────────────────────────────┘
```
...
AgentOrchestrator.processUserInput()
    ...
    │   └── 100% 端侧推理，零网络依赖，符合 [PRIVACY] 红线
    ├── 远程模式（REMOTE）：InferenceRouter → RemoteOrchestrator
    │   └── 云端 LLM 编排，仅 PUBLIC 级别指令允许路由
...
**关键设计决策**：
- **InferenceRouter 自适应路由**：根据 PrivacyGuard 分级、网络状态、模型可用性自动选择本地或远程推理
...
| 端侧 LLM 可满足相机控制推理需求 | ✅ 成立 — Qwen3-1.7B 本地推理响应 < 500ms |
| 隐私分级守卫可有效拦截敏感数据 | ✅ 成立 — RESTRICTED/SENSITIVE 指令 100% 本地执行 |
| 文字交互模式稳定性 | ✅ 成立 — 自然语言控制相机功能已稳定可用 |
| 语音控制可行性 | 🔄 验证中 — 唤醒词 + 声控链路已跑通，精度和体验持续优化 |
...
5. **语音交互体验**：唤醒词识别精度、远场拾音、噪声抑制能否达到产品级标准？
6. **云端推理策略**：自适应路由策略（本地/远程/混合）如何在延迟、成本、效果间取得最优平衡？
