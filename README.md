...
- 静态 Bitmap 美颜由 `GpuBeautyProcessor`（CPU 路径，位于 `core/image/`）完成，与实时预览 GPU 路径完全隔离
- 磨皮：Canvas + ColorMatrix 亮度近似（原 RenderScript 已因 API 废弃替换）
- 色调滤镜：`FilterType.toAndroidColorMatrix()` 应用于保存前的 Bitmap
