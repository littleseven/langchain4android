...
> **实验状态**：进行中 · Phase 3 对话中心转型（聊天首页 + 模型切换 + 对话持久化）
...
**当前聚焦**：从「相机工具」向「AI 对话助手」转型，以聊天首页为核心入口，相册/编辑/相机作为能力插件被 Agent 调度。
...
5. **通用 Agent**：不做跨应用调用、不做系统级自动化，仅聚焦相册+编辑+对话场景
...
**关键决策**：Agent 层是"唯一可信源"，UI 是对 Agent 状态的反映。聊天首页是新的「唯一可信源」入口，所有能力（相机/相册/编辑）通过对话触发。
...
- LLM：Qwen3.5-2B 通过 MNN-LLM 本地运行（模型升级）
...
| 远程混合编排 | ✅ | InferenceRouter L1~L4 策略分层，端侧专注 L1/L2，L3/L4 远程或规则 |
...
```
AI 对话（核心入口）→ 相册+编辑（核心能力）→ 拍照（能力基线）→ 视频（极限挑战）
                              ↓
...
| 功能 | 状态 | 优先级 | 说明 |
|------|------|--------|------|
| **聊天首页** | 🔄 开发中 | P0 | 独立 ChatScreen，作为应用首页 |
| **本地模型对话** | ✅ 已落地 | P0 | Qwen3.5-2B 端侧推理，基础对话+指令解析 |
| **远程模型对话** | ✅ 已落地 | P0 | DeepSeek 等远程 LLM，复杂推理 |
...
| 零延迟快门 | ✅ 已落地 | P1 | < 50ms 快门延迟，三位反馈 |
| 场景识别 | ✅ 已落地 | P1 | 夜景/月亮自动触发 |
| 风格特效 | ✅ 已落地 | P1 | 卡通/素描/浮雕/色块/交叉线 |
| 帧同步美妆 | ✅ 已落地 | P1 | 解决妆容甩飞，支撑视频美颜 |
| 智能场景模板 | 📋 规划中 | P2 | 一键切换「清透日常/精致约会/酷飒街拍」 |
| 证件照模式 | 📋 规划中 | P2 | 自动抠图换背景、美颜降级 |
| 连拍优选 | 📋 规划中 | P3 | 表情识别自动标记最佳照片 |
| RAW 格式支持 | 📋 规划中 | P3 | 专业摄影用户 |

**演进路线**：
- **Phase 1（近期）**：保持现有能力，从聊天页提供快捷入口
...
| 阶段 | 时间 | 目标 | 关键交付 |
|------|------|------|----------|
| **Phase 1** | 近期（2-4 周） | 对话中心转型 | 聊天首页落地；模型切换 UI；对话持久化；快捷入口栏；AI 一键优化 + 精准局部美颜 |
| **Phase 2** | 中期（4-8 周） | 智能体验升级 | 图片消息支持；对话式编辑；智能消除；专业调色；实时美颜录制（1080p@30fps）|
| **Phase 3** | 长期（8-16 周） | 创作者生态 | 多会话管理；证件照/连拍/RAW；智能运镜/Vlog/多平台导出；拼图/Pro 功能 |

**资源分配建议**：
- Phase 1：对话 40% / 编辑 35% / 拍照 15% / 视频 10%（对话首页是转型核心，优先投入）
- Phase 2：对话 30% / 编辑 35% / 视频 25% / 拍照 10%（编辑+视频是增长重点）
- Phase 3：对话 25% / 编辑 40% / 视频 25% / 拍照 10%（编辑是变现核心，重点打磨）
