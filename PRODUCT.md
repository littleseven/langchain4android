...
**关键认知更新（2026-06-12）**：端侧 Qwen3-1.7B 模型验证结果表明，其能力边界为「指令路由器」而非「推理引擎」。Function Calling 和简单指令翻译胜任，但复杂推理、多步规划、上下文推理超出能力范围。产品策略据此调整：端侧专注 L1/L2（缓存+单条/简单批量 FC），复杂推理明确上云或降级为规则引擎。
...
1. **云端服务**：不接入云端存储/同步；云 LLM 仅作为非敏感指令的降级兜底，敏感数据（人脸/对话）100% 端侧
...
| 功能 | 状态 | 优先级 | 说明 |
|------|------|--------|------|
| **基础视频录制** | 🔄 部分落地 | P0 | BeautyVideoRecorder 已落地，UI 触发入口待补充 |
| **实时美颜录制** | 🔄 部分落地 | P0 | FrameSyncManager 复用预览链路，录制时妆容不甩飞 |
| **帧同步美妆（视频）** | 🔄 部分落地 | P0 | 录制链路复用同一 FrameSyncManager 实例 |
