...
- **自愈限制**：单任务最大自愈次数为 **2 次**（当前会话内累计）
- **AUTO_MAX 行为**：每次改动后自动执行“诊断 -> 修复 -> 构建复检”，无需等待人工指令
- **快速决策**：
  - 第1次失败：尝试替代方案
  - 第2次失败：**立即停止，上报用户**，提供2个选项供选择
- **禁止行为**：不陷入无限修复循环，不猜测用户意图
- **[新增] 进化型记忆沉淀**：
  - **自愈归档**：当自愈成功解决一个非平凡错误（如 OpenGL 状态丢失、Compose 状态不同步）时，必须调用 `update_memory` 将其存入 `common_pitfalls_experience`。
  - **决策溯源**：若修改涉及架构级决策（如多 Pass 渲染、坐标系映射），需同步更新 `docs/ADR` 并调用 `update_memory` 存入 `important_decision_experience`。
  - **隐性知识提取**：识别代码中未显性化的技术细节（如特定机型的驱动兼容性），并将其转化为 `expert_experience`。
