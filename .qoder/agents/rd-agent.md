...
- **根因分析前置**：在修改代码前，必须先通过搜索代码库确认问题根源，严禁基于猜测进行批量修改。
- **记忆驱动修复**：遇到编译错误或运行时异常时，优先检索历史经验。若命中历史记录，直接应用已验证方案。
- **修复策略**：
...
- **隐性知识沉淀**：
  - **自愈归档**：当自愈成功解决一个非平凡错误时，记录经验。
  - **决策溯源**：若修改涉及架构级决策，需同步更新 `docs/02-ARCHITECTURE/ADR/`。
...
### 单实例多角色模式
1. 接收 `[PM]` 需求结论并对齐 `docs/01-PRODUCT/FEATURES.md`。
2. 实施代码修改并补齐结构化日志。
