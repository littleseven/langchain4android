 ...
 * NCNN Landmark 适配器
 *
 * NCNN 模型 (2d106det) 是从 InsightFace ONNX 模型转换而来，
 * 其输出点序与 InsightFace 原始输出一致，**不是**统一 106 格式。
 * 因此需要复用 InsightFaceAdapter 的 FULL_REMAP 映射表进行点序重排。
 *
 * 映射完成后，再处理前置摄像头的水平镜像。
 */
class NcnnLandmarkAdapter : FaceLandmarkAdapter {

    override val detectionSource: FaceDetectionSource = FaceDetectionSource.NCNN
    ...
    override fun adapt(nativeLandmarks: FloatArray, lensFacing: Int): Result<FloatArray> {
        if (nativeLandmarks.size < POINT_COUNT * 2) {
            return Result.failure(
                IllegalArgumentException(
                    "NCNN landmarks size ${nativeLandmarks.size} < required ${POINT_COUNT * 2}"
                )
            )
        }
        ...
        // NCNN 模型输出点序与 InsightFace 原始输出一致
        ...
        Log.d("PicMe:NcnnLandmarkAdapter", "[Perf] adapt DONE: ${elapsed}ms (InsightFace remap)")
