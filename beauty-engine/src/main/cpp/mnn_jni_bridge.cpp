...
JNIEXPORT jlong JNICALL
Java_com_mamba_picme_beauty_internal_clip_MobileClipEncoder_nativeCreate(
        JNIEnv *env,
        jclass clazz) {
    auto *encoder = new picme::MobileClipEncoder();
    LOGD("MobileClipEncoder created");
    return reinterpret_cast<jlong>(encoder);
}

JNIEXPORT void JNICALL
Java_com_mamba_picme_beauty_internal_clip_MobileClipEncoder_nativeDestroy(
        JNIEnv *env,
        jclass clazz,
        jlong handle) {
    auto *encoder = reinterpret_cast<picme::MobileClipEncoder *>(handle);
    delete encoder;
    LOGD("MobileClipEncoder destroyed");
}

JNIEXPORT jboolean JNICALL
Java_com_mamba_picme_beauty_internal_clip_MobileClipEncoder_nativeLoadVisionModel(
        JNIEnv *env,
        jclass clazz,
        jlong handle,
        jstring modelPath,
    ...
    bool success = encoder->loadVisionModel(pathCStr, useGpu);
    env->ReleaseStringUTFChars(modelPath, pathCStr);

    LOGD("Load vision model: %s", success ? "success" : "failed");
    return success ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_mamba_picme_beauty_internal_clip_MobileClipEncoder_nativeLoadTextModel(
        JNIEnv *env,
        jclass clazz,
        jlong handle,
        jstring modelPath,
    ...
    const char *pathCStr = env->GetStringUTFChars(modelPath, nullptr);
    bool success = encoder->loadTextModel(pathCStr, useGpu);
    env->ReleaseStringUTFChars(modelPath, pathCStr);

    LOGD("Load text model: %s", success ? "success" : "failed");
    return success ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jfloatArray JNICALL
Java_com_mamba_picme_beauty_internal_clip_MobileClipEncoder_nativeEncodeImage(
        JNIEnv *env,
        jclass clazz,
        jlong handle,
    ...
    unsigned char *data = static_cast<unsigned char *>(env->GetDirectBufferAddress(imageData));
    if (!data) {
        LOGE("nativeEncodeImage: GetDirectBufferAddress returned null");
        return nullptr;
    }

    std::vector<float> result = encoder->encodeImage(data, width, height);
    if (result.empty()) {
        return nullptr;
    }
...
JNIEXPORT jfloatArray JNICALL
Java_com_mamba_picme_beauty_internal_clip_MobileClipEncoder_nativeEncodeText(
        JNIEnv *env,
        jclass clazz,
        jlong handle,
        jlongArray tokenIds,    // int64 token IDs
        jint tokenCount) {
    auto *encoder = reinterpret_cast<picme::MobileClipEncoder *>(handle);
    if (!encoder) return nullptr;

    jlong *tokens = env->GetLongArrayElements(tokenIds, nullptr);
    if (!tokens) {
        LOGE("nativeEncodeText: GetLongArrayElements returned null");
        return nullptr;
    }

    // jlong (int64_t) -> int64_t
    std::vector<int64_t> tokenVec(tokenCount);
    for (int i = 0; i < tokenCount; i++) {
        tokenVec[i] = static_cast<int64_t>(tokens[i]);
    }
    env->ReleaseLongArrayElements(tokenIds, tokens, JNI_ABORT);

    std::vector<float> result = encoder->encodeText(tokenVec.data(), tokenCount);
    if (result.empty()) {
        return nullptr;
    }
...
JNIEXPORT jboolean JNICALL
Java_com_mamba_picme_beauty_internal_clip_MobileClipEncoder_nativeIsVisionLoaded(
        JNIEnv *env,
        jclass clazz,
        jlong handle) {
    auto *encoder = reinterpret_cast<picme::MobileClipEncoder *>(handle);
    return (encoder && encoder->isVisionLoaded()) ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_mamba_picme_beauty_internal_clip_MobileClipEncoder_nativeIsTextLoaded(
        JNIEnv *env,
        jclass clazz,
        jlong handle) {
    auto *encoder = reinterpret_cast<picme::MobileClipEncoder *>(handle);
    return (encoder && encoder->isTextLoaded()) ? JNI_TRUE : JNI_FALSE;
}
