...
| 特性 | MobileCLIP-S2 | 说明 |
|------|---------------|------|
| 模型大小 | ~143MB (vision) + ~242MB (text) | 端侧可承受 |
| 推理速度 | ~50-100ms/张 (vision) | ONNX Runtime 后端，CPU 即可 |
...
- **[复用]**：复用已有 ONNX Runtime 推理栈、模型中心下载系统、Room 数据库
...
从 ModelScope (`budaoshou/MobileCLIP-ONNX`) 下载 ONNX 版模型：

| 文件名 | 大小 | 用途 | 必需 | 说明 |
|--------|------|------|------|------|
| `vision_model.onnx` | ~143MB | 图像编码器 | ✅ | 纯 fp32 ONNX，CPU 可稳定推理 |
| `text_model.onnx` | ~242MB | 文本编码器 | ✅ | 纯 fp32 ONNX |
| `tokenizer.json` | ~2.2MB | Tokenizer 数据 | ✅ | Hugging Face 格式 |

> 存储路径：`/sdcard/Android/data/com.mamba.picme/files/llm_models/mobileclip-onnx/`
>
> **注意**：仓库中同时提供 `*_fp16.onnx` / `*_fp32.onnx` 命名文件，但内部仍是 fp16 计算，ONNX Runtime Android CPU/NNAPI 上无法初始化。务必使用不带后缀的 `vision_model.onnx` / `text_model.onnx`。
...
├── MobileClipOnnxBackend.kt     # ONNX Runtime 推理后端
├── MobileClipTokenizer.kt       # BPE Tokenizer
└── TagGenerationPipeline.kt     # 集成 Pass 1/4
```

**MobileClipEngine** 职责：
- 模型存在性检查（`isModelReady`）
- ONNX 模型加载/卸载（`initialize()` / `release()`）
...
MobileCLIP 文本编码使用 `MobileClipTokenizer` 解析 `tokenizer.json`，生成 token IDs 后交给 `MobileClipOnnxBackend.encodeText(tokenIds)`：

```
用户输入: "温馨的家庭聚餐"
    │
    ▼
MobileClipTokenizer (Hugging Face tokenizer.json)
    ▼
Token IDs: [49406, 1234, 5678, ...]
    │
    ▼
MobileClipOnnxBackend.encodeText(tokenIds) → 512-dim embedding
```
...
| **GPU 加速** | 实时搜索 | 低（ONNX Runtime NNAPI 支持 GPU 回退） |
