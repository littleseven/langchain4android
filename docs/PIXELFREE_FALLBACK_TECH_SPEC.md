...
- 大美丽 主链路（Provider 渲染、回退状态机、冷却恢复）见 `BIG_BEAUTY_TECH_SPEC.md`。
- 预览比例与坐标映射见 `CAMERA_PREVIEW_TECH_SPEC.md`。

## 1. 技术选型决策

### 1.1 背景
当前项目采用双引擎策略：`BIG_BEAUTY` 作为默认主引擎，`PIXEL_FREE` 作为稳定兜底引擎。PixelFree 文档用于说明兜底链路的集成、维护与兼容性要求；大美丽 主链路详见 `BIG_BEAUTY_TECH_SPEC.md`。

### 1.2 双轨策略

根据当前技术路线，双轨策略调整为：

- **主引擎（默认）**：大美丽（自研 OpenGL 链路）
- **兜底引擎**：PixelFreeEffects（异常回退、稳定可用）
- **恢复机制**：冷却窗口结束后自动重试 大美丽，失败则继续回退 PixelFree
...
| 方案 | 角色定位 | 优势 | 风险 |
|------|----------|------|------|
| 大美丽（自研） | 默认主引擎 | 可控性高、可观测性强、授权成本低 | 设备兼容与预览链路复杂度高 |
| PixelFreeEffects | 稳定兜底 | 兼容性成熟、故障回退快 | 授权与扩展受限 |

**技术路线说明**：
- **运行策略**：默认使用 大美丽，异常自动回退 PixelFree。
