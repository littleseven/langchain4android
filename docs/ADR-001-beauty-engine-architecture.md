...
| **依赖方向** | App → api → impl → GPUPixel (单向) |
| **接口定义** | `BeautyEngine` 接口类，对外唯一出口 |
| **GPUPixel封装** | Adapter模式，`GPUPixelAdapter` 统一封装所有GL调用 |
| **线程模型** | HandlerThread GL线程，impl层管理独立EGLContext |
...
├── impl/                   # Data Layer - 内部实现
│   ├── BeautyEngineImpl.kt # 接口实现
│   ├── GPUPixelAdapter.kt  # EGL/GL 完整封装
│   └── BigBeautyEngine.kt  # 自研引擎
```
...
- `architecture-test/ModuleDependencyTest.kt` - 架构合规测试
