...
**最后更新**：2026-04-18（新增 MediaPipe 468 → GPUPixel 111 点映射规范，修正业界 106 点为实际 111 点）
...
### 3.5 难点 5：MediaPipe 468 点 → GPUPixel 111 点映射

**背景**：大美丽模式使用 MediaPipe Face Landmarker（468 点）替代 ML Kit，需将 468 点映射为与 GPUPixel/字节火山引擎兼容的 111 点格式（索引 0-110）。

**关键发现**：GPUPixel C++ 源码（`face_makeup_filter.cc`）中 `GetFaceIndexs()` 实际使用索引 0-110，共 111 点，而非业界常说的 106 点。索引 106-110 为三角形网格填充辅助点。

#### 3.5.1 GPUPixel 111 点拓扑定义

| 索引范围 | 点数 | 区域 | 说明 |
|---------|------|------|------|
| 0-32 | 33 | 脸部轮廓 | 开放曲线：左脸颊 → 下巴 → 右脸颊，无额头点 |
| 33-37 | 5 | 左眉 | 从外到内（33=左外，37=左内） |
| 38-42 | 5 | 右眉 | 从内到外（38=右内，42=右外） |
| 43 | 1 | 眉心 | 两眉之间中点 |
| 44-46 | 3 | 鼻梁 | 从上到下（44=鼻根，46=鼻尖上） |
| 47-51 | 5 | 鼻尖 | 从左到右 |
| 52-57 | 6 | 左眼外轮廓 | 从左外角顺时针 |
| 58-63 | 6 | 右眼外轮廓 | 从右内角顺时针 |
| 64-67 | 4 | 左眉下辅助 | 眉毛下方过渡区域 |
| 68-71 | 4 | 右眉下辅助 | 眉毛下方过渡区域 |
| 72-74 | 3 | 左眼内/下 | 74=左瞳孔 |
| 75-77 | 3 | 右眼内/下 | 77=右瞳孔 |
| 78-79 | 2 | 鼻孔上 | 鼻孔上方边缘 |
| 80-83 | 4 | 鼻孔 | 左右鼻孔轮廓 |
| 84-95 | 12 | 嘴巴外轮廓 | 上唇6点+下唇6点 |
| 96-103 | 8 | 嘴巴内轮廓 | 上唇内4点+下唇内4点 |
| 104-105 | 2 | 瞳孔重复 | 与 74/77 同位置 |
| 106-110 | 5 | 辅助点 | 三角形网格填充用 |

#### 3.5.2 MediaPipe 468 → 111 点映射表

**轮廓 33 点（0-32）**：由 MediaPipe FACE_OVAL 左半边点插值 + 强制对称生成，非直接映射。

**非轮廓 73 点（33-105）**：

```
左眉 33-37:      70, 63, 105, 66, 55
右眉 38-42:      285, 293, 334, 296, 300
眉心 43:         168
鼻梁 44-46:      6, 197, 195
鼻尖 47-51:      98, 241, 2, 461, 327
左眼外 52-57:    33, 246, 161, 160, 159, 158
右眼外 58-63:    362, 398, 384, 385, 386, 387
左眉下 64-67:    156, 157, 173, 155
右眉下 68-71:    383, 382, 381, 380
左眼内 72-74:    133, 153, 154
右眼内 75-77:    388, 466, 263
鼻孔上 78-79:    98, 326
鼻孔 80-83:      2, 327, 358, 359
嘴外 84-95:      61, 185, 40, 39, 37, 0, 267, 269, 270, 409, 375, 321
嘴内 96-103:     405, 314, 17, 84, 181, 91, 146, 96
瞳孔重复 104-105: 145, 374
```
