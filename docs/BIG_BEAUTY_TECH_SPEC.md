...
- **主引擎（默认）**：大美丽（`BIG_BEAUTY`）
- **备用引擎**：PixelFreeEffects SDK（`PIXEL_FREE`）
- **切换方式**：设置页「美颜引擎」配置开关
- **容灾策略**：大美丽初始化失败或运行异常时，自动回退 PixelFreeEffects
- **长期定位**：大美丽 从 App 内部能力逐步演进为独立视觉能力基础库。
...
4. **容错可用**：大美丽 预览链路失败时自动切换 PixelFree 兜底，并在冷却结束后自动重试 大美丽
...
| 维度 | PixelFreeEffects SDK | 大美丽（自研） |
|------|----------------------|----------------|
...
- [ ] 持续迭代 `:beauty-engine` 模块：大美丽 渲染适配层（Surface/CameraX/OpenGL）。
...
- 当前冷却窗口：`BIG_BEAUTY_RECOVERY_COOLDOWN_MS = 3 * 60 * 1000L`。
...
为避免 `BIG_BEAUTY_TECH_SPEC.md` 过长，QA 相关内容已提取到独立文档：

- `docs/BIG_BEAUTY_QA_EXECUTION_CHECKLIST.md`
...
- 建立"兼容性问题库"，记录已知问题
- 针对特定设备禁用高级特性
- 自动回退到 PixelFree 兜底，并在冷却后自动重试 大美丽
...
- `app/src/main/java/com/picme/core/image/BeautyPreviewView.kt` - 大美丽 预览视图实现
- `app/src/main/java/com/picme/core/image/CameraPreviewRenderer.kt` - 大美丽 渲染主链路
...
5. **自动回退**：大美丽 异常时切换 PixelFree，并进入冷却后自动重试
...
- 失败：自动切换 PixelFree 兜底，并在冷却窗口结束后自动重试 大美丽
