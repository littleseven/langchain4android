...
2. 人工注入初始化失败（例如抛异常），确认回退到 `PreviewView`（无美颜预览）。
3. 验证持久化写入：`gl_engine_fallback_reason` 与 `gl_engine_recovery_available_at_ms` 已写入。
...
- [ ] `P0-03` 大美丽 warm-up 失败可自动回退到 PreviewView（无美颜预览），且拍照功能可用。
...
| P0-03 | unit + androidTest | `CameraP0AutomationSkeletonTest` + `BeautyPreviewProviderFactoryTest` | 大美丽 init 失败后回落 `PreviewView`，且 `useProviderRenderView=false`，持久化状态正确 | 已建骨架 |
