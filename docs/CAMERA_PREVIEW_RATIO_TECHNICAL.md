...
### 9.1 垂直拉伸问题（2026-03-31 修复）

**问题现象**：
- 4:3 比例正常，但 16:9 和 FULL 模式出现垂直拉伸
- 人物变得细长，画面比例明显不对

**错误原因**：
```kotlin
// ❌ 错误：使用了横向比例（1920/1080 = 1.778）
val newHeight = (width * (1920f / 1080f)).toInt()  // 比例倒置！
```

**问题分析**：
- 相机传感器输出 1920x1080（横向）
- 旋转 90° 后变成 1080x1920（竖向）
- TextureView 应该匹配**竖向比例**：宽/高 = 1080/1920 = **0.5625**
- 错误地使用了 1920/1080 = 1.778，导致高度不足，画面被压缩

**正确修复**：
```kotlin
// ✅ 正确：使用竖向比例（9/16 = 0.5625）
if (height > width) {
    // 竖屏：宽度固定，高度 = 宽度 / 0.5625
    val newHeight = (width / 0.5625f).toInt()
    setMeasuredDimension(width, newHeight)
} else {
    // 横屏：高度固定，宽度 = 高度 * 0.5625
    val newWidth = (height * 0.5625f).toInt()
    setMeasuredDimension(newWidth, height)
}
```

**关键要点**：
- 永远使用**旋转后的比例**，不是传感器原始比例
- 竖屏时：宽/高 = 9/16 = 0.5625
- 横屏时：宽/高 = 16/9 = 1.778
