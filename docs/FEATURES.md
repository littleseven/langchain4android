...
##### 1.3.4.5 大美丽 引擎策略与用户体验
- **双引擎共存**：大美丽与 PixelFreeEffects 长期保留，支持配置开关切换。
- **默认引擎**：大美丽（自主实现，性能与可控性优先）。
- **备用引擎**：PixelFreeEffects（稳定兜底）。
- **切换入口**：设置页「美颜引擎」单选项（即时生效，切换后立即影响预览）。
- **故障回退**：大美丽初始化失败或运行中异常时，自动回退至 PixelFreeEffects。
- **用户提示**：发生自动回退时，以轻提示告知“已切换稳定模式，可在设置中重试 大美丽”。
- **可恢复性**：异常恢复后允许用户手动切回 大美丽，不需要重启应用。
- **技术实现文档**：引擎实现、回退状态机与渲染链路细节见 [`BIG_BEAUTY_TECH_SPEC.md`](BIG_BEAUTY_TECH_SPEC.md) 与 [`PIXELFREE_FALLBACK_TECH_SPEC.md`](PIXELFREE_FALLBACK_TECH_SPEC.md)。

### 1.3.5 大美丽 实时预览性能与调试控制（2026-04 更新）
