...
class MediaPipeFaceDetector(context: Context) {
    ...
    companion object {
        ...
        //   33-37: 右眉上部5点（从眉头到眉尾，图像左侧）
        //   38-42: 左眉上部5点（从眉头到眉尾，图像右侧）
        //   43: 眉心
        //   44-46: 鼻梁3点（从上到下）
        //   47-51: 鼻尖5点（从左到右）
        //   52-57: 左眼外轮廓6点（左外角→内角顺时针）
        //   58-63: 右眼外轮廓6点（右外角→内角顺时针）
        //   64-67: 右眉下部4点（从左到右）
        //   68-71: 左眉下部4点（从左到右）
        ...
        private val NON_CONTOUR_MAPPING = intArrayOf(
            // === 右眉上部 33-37 (5点) - 从眉头到眉尾（图像左侧） ===
            // 基于 MediaPipe 右眉语义关键点（索引 70-79, 107-113）：
            //   70=眉头, 63=眉腰前, 105=眉峰, 66=眉腰后, 107=眉尾
            70, 63, 105, 66, 107,

            // === 左眉上部 38-42 (5点) - 从眉头到眉尾（图像右侧） ===
            // 基于 MediaPipe 左眉语义关键点（与右眉对称）：
            //   336=眉头, 296=眉腰前, 334=眉峰, 293=眉腰后, 300=眉尾
            336, 296, 334, 293, 300,
            ...
            // === 鼻尖 47-51 (5点) - 从左到右 ===
            // 核心点：4=鼻尖中心
            // 策略：使用MediaPipe鼻子关键区域点
            //   47=左鼻翼上, 48=鼻尖左, 49=鼻尖中心, 50=鼻尖右, 51=右鼻翼上
            //   根据MediaPipe官方拓扑：48(左), 4(中), 51(右)为鼻尖主要点
            48, 4, 4, 4, 51,

            // === 右眼外轮廓 52-57 (6点) - 画面左侧=实际右脸，从外角顺时针 ===
            // 核心点：362=右眼外角, 463=右眼内角
            // 策略：沿上眼睑从外角到内角
            //   362=外角, 385=上眼睑外, 386=上眼睑中1, 387=上眼睑中2, 388=上眼睑内, 463=内角
            362, 385, 386, 387, 388, 463,

            // === 左眼外轮廓 58-63 (6点) - 画面右侧=实际左脸，从外角顺时针 ===
            // 核心点：33=左眼外角, 133=左眼内角
            // 策略：沿上眼睑从外角到内角
            //   33=外角, 157=上眼睑外, 158=上眼睑中1, 160=上眼睑中2, 161=上眼睑内, 133=内角
            33, 157, 158, 160, 161, 133,

            // === 右眉下部 64-67 (4点) - 从眉头到眉尾（图像左侧） ===
            // 基于 MediaPipe 右眉下语义关键点：
            //   46=眉头下, 53=眉腰前下, 52=眉峰下, 65=眉尾下
            46, 53, 52, 65,

            // === 左眉下部 68-71 (4点) - 从眉头到眉尾（图像右侧） ===
            // 基于 MediaPipe 左眉下语义关键点（与右眉对称）：
            //   276=眉头下, 283=眉腰前下, 282=眉峰下, 295=眉尾下
            276, 283, 282, 295,

            // === 右眼内/下 72-74 (3点) - 72=右眼内角, 73=下眼睑中, 74=右瞳孔 ===
            // 画面左侧=实际右脸，使用MediaPipe右眼关键点
            //   463=右眼内角(靠近鼻梁), 374=右眼下眼睑中, 473=右瞳孔中心(478点模式)
            463, 374, 473,

            // === 左眼内/下 75-77 (3点) - 75=左眼内角, 76=下眼睑中, 77=左瞳孔 ===
            // 画面右侧=实际左脸，使用MediaPipe左眼关键点
            //   133=左眼内角(靠近鼻梁), 145=左眼下眼睑中, 468=左瞳孔中心(478点模式)
            133, 145, 468,

            // === 鼻梁底部 78-79 (2点) - 山根/鼻梁起点 ===
            // 基于 MediaPipe 鼻子语义关键点：
            //   6=鼻根/山根(两眼之间鼻梁起点), 5=鼻底中(山根下方)
            //   注意：78=山根左, 79=山根右（使用相同点表示山根中心）
            6, 5,

            // === 鼻孔 80-83 (4点) - 80=左鼻孔左, 81=左鼻孔右, 82=右鼻孔左, 83=右鼻孔右 ===
            // 基于 MediaPipe 鼻子语义关键点：
            //   2=左鼻孔左, 327=左鼻孔右, 358=右鼻孔左, 359=右鼻孔右
            2, 327, 358, 359,

            // === 嘴巴外轮廓 84-95 (12点) - 从左嘴角顺时针 ===
            // 核心点：61=左嘴角, 0=唇珠, 291=右嘴角
            // 策略：上唇6点(84-89) + 下唇6点(90-95)
            // 上唇：61→185→40→39→37→0 (左嘴角到唇珠)
            // 下唇：267→291→375→321→409→91 (唇珠右到左嘴角下)
            61, 185, 40, 39, 37, 0, 267, 291, 375, 321, 409, 91,

            // === 嘴巴内轮廓 96-103 (8点) - 从左内角顺时针 ===
            // 核心点：78=左内角, 87=右内角, 308=下唇内左, 310=下唇内右
            // 策略：上唇内4点 + 下唇内4点
            // 上唇内：78→95→88→87 (左上到右上)
            // 下唇内：308→318→402→310 (左下到右下)
            78, 95, 88, 87, 308, 318, 402, 310,

            // === 瞳孔 104-105 (2点) - 104=左瞳孔(与74同位), 105=右瞳孔(与77同位) ===
            // 使用与74/77相同的MediaPipe点，确保位置一致
            468, 473
        )
