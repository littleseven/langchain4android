...
class MediaPipeFaceDetector(context: Context) {
    ...
    companion object {
        ...
        private val NON_CONTOUR_MAPPING = intArrayOf(
            ...
            // MediaPipe右眼路径（从外角到内角）：362→398→384→385→386→387→388→466→463
            //   52=362(外角), 53=398(上眼睑外), 54=386(上眼睑中), 55=388(上眼睑内)
            //   56=463(内角/上眼睑终点), 57=372(下眼睑内角)
            // 上眼睑3点(53-55)均匀分布，不含眼角
            362, 398, 386, 388, 463, 372,

            // === 左眼 58-63 (6点) - 画面右侧=实际左脸 ===
            // 与右眼对称映射：
            //   58=33(外角), 59=246(上眼睑外), 60=159(上眼睑中), 61=157(上眼睑内)
            //   62=133(内角), 63=155(下眼睑内角)
            // 对称性：53(398)↔55(388)关于眼中心对称，59(246)↔61(157)关于眼中心对称
            33, 246, 159, 157, 133, 155,
            ...
            // MediaPipe对应：374=下眼睑中, 380=下眼睑外, 473=右瞳孔
            // 下眼睑内角在57(372)，与72-74组成完整下眼睑3点(不含内角)
            374, 380, 473,

            // === 左眼内/下 75-77 (3点) - 画面右侧 ===
            // 下眼睑3点(不含角)：75=下眼睑中, 76=下眼睑外, 77=瞳孔
            // MediaPipe对应：145=下眼睑中, 144=下眼睑外, 468=左瞳孔
            // 下眼睑内角在63(155)，与75-76组成完整下眼睑3点(不含内角)
            145, 144, 468,
