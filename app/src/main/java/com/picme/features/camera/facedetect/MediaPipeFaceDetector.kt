 ...
 *
 * 106点拓扑方向（画面视角，前置摄像头镜像后）：
 *   轮廓：从右鬓角(0) → 下巴(16) → 左鬓角(32)，开放曲线
 *   眉毛/眼睛：画面左侧=实际右脸，画面右侧=实际左脸
 *
...
class MediaPipeFaceDetector(context: Context) {
    ...
    companion object {
        ...
        // 106点标准拓扑（画面视角，前置摄像头镜像后）：
        //   0-16: 左轮廓17点（从左上到下巴左）
        //   17-32: 右轮廓16点（从下巴右到右上）
        //   33-37: 右眉上部5点（从眉头到眉尾，画面左侧=实际右脸）
        //   38-42: 左眉上部5点（从眉头到眉尾，画面右侧=实际左脸）
        //   43: 眉心
        //   44-46: 鼻梁3点（从上到下）
        //   47-51: 鼻尖5点（从左到右）
        //   52-57: 右眼外轮廓6点（画面左侧=实际右脸，从外角→内角顺时针）
        //   58-63: 左眼外轮廓6点（画面右侧=实际左脸，从外角→内角顺时针）
        //   64-67: 右眉下部4点（从左到右，画面左侧）
        //   68-71: 左眉下部4点（从左到右，画面右侧）
        //   72-74: 右眼内/下3点（72=内角，73=下眼睑中，74=右瞳孔）
        //   75-77: 左眼内/下3点（75=内角，76=下眼睑中，77=左瞳孔）
        //   78-79: 山根2点（鼻梁起点左右）
        //   80-83: 鼻孔4点（80=左鼻孔左，81=左鼻孔右，82=右鼻孔左，83=右鼻孔右）
        //   84-95: 嘴巴外轮廓12点（84=左嘴角，90=唇珠，94=右嘴角）
        //   96-103: 嘴巴内轮廓8点（96=左内角，100=右内角）
        //   104: 右瞳孔（与74同位置）
        //   105: 左瞳孔（与77同位置）
        // 非轮廓区域映射表（33-105，共73点）
        private val NON_CONTOUR_MAPPING = intArrayOf(
            // === 右眉上部 33-37 (5点) - 从眉头到眉尾（画面左侧=实际右脸） ===
            // 基于 MediaPipe 右眉语义关键点（观察者视角的"右眉"，位于画面左侧）：
            //   336=眉头(靠近鼻梁/画面左内侧), 296=眉腰前, 334=眉峰, 293=眉腰后, 300=眉尾
            336, 296, 334, 293, 300,

            // === 左眉上部 38-42 (5点) - 从眉头到眉尾（画面右侧=实际左脸） ===
            // 基于 MediaPipe 左眉语义关键点（观察者视角的"左眉"，位于画面右侧）：
            ...
            // === 右眼外轮廓 52-57 (6点) - 画面左侧=实际右脸，从外角→内角顺时针 ===
            // 核心点：362=右眼外角(画面左侧), 463=右眼内角(靠近鼻梁)
            // 策略：沿上眼睑从外角到内角
            //   362=外角, 385=上眼睑外, 386=上眼睑中1, 387=上眼睑中2, 388=上眼睑内, 463=内角
            362, 385, 386, 387, 388, 463,

            // === 左眼外轮廓 58-63 (6点) - 画面右侧=实际左脸，从外角→内角顺时针 ===
            // 核心点：33=左眼外角(画面右侧), 133=左眼内角(靠近鼻梁)
            // 策略：沿上眼睑从外角到内角
            //   33=外角, 157=上眼睑外, 158=上眼睑中1, 160=上眼睑中2, 161=上眼睑内, 133=内角
            33, 157, 158, 160, 161, 133,

            // === 右眉下部 64-67 (4点) - 从眉头到眉尾（画面左侧=实际右脸） ===
            // 基于 MediaPipe 右眉下语义关键点（观察者视角"右眉"，画面左侧）：
            //   276=眉头下, 283=眉腰前下, 282=眉峰下, 295=眉尾下
            276, 283, 282, 295,

            // === 左眉下部 68-71 (4点) - 从眉头到眉尾（画面右侧=实际左脸） ===
            // 基于 MediaPipe 左眉下语义关键点（观察者视角"左眉"，画面右侧）：
            //   46=眉头下, 53=眉腰前下, 52=眉峰下, 65=眉尾下
            46, 53, 52, 65,

            // === 右眼内/下 72-74 (3点) - 画面左侧=实际右脸 ===
            //   72=右眼内角(靠近鼻梁), 73=右眼下眼睑中, 74=右瞳孔
            // MediaPipe对应：463=右眼内角, 374=右眼下眼睑中, 473=右瞳孔中心(478点模式)
            463, 374, 473,

            // === 左眼内/下 75-77 (3点) - 画面右侧=实际左脸 ===
            //   75=左眼内角(靠近鼻梁), 76=左眼下眼睑中, 77=左瞳孔
            // MediaPipe对应：133=左眼内角, 145=左眼下眼睑中, 468=左瞳孔中心(478点模式)
            133, 145, 468,

            // === 鼻梁底部/山根 78-79 (2点) ===
            // 基于 MediaPipe 鼻子语义关键点：
            //   78=山根左(鼻梁起点左侧), 79=山根右(鼻梁起点右侧)
            ...
            // === 嘴巴外轮廓 84-95 (12点) - 从左嘴角顺时针 ===
            // 核心点：61=左嘴角, 0=唇珠/上唇中心, 291=右嘴角
            // 策略：上唇6点(84-89) + 下唇6点(90-95)
            // 上唇：61(左嘴角)→185→40→39→37→0(唇珠)
            // 下唇：267→291(右嘴角)→375→321→409→91(左嘴角下)
            61, 185, 40, 39, 37, 0, 267, 291, 375, 321, 409, 91,

            // === 嘴巴内轮廓 96-103 (8点) - 从左内角顺时针 ===
            // 核心点：78=左内角, 87=右内角, 308=下唇内左, 310=下唇内右
            // 策略：上唇内4点 + 下唇内4点
            // 上唇内：78(左上)→95→88→87(右上)
            // 下唇内：308(左下)→318→402→310(右下)
            78, 95, 88, 87, 308, 318, 402, 310,

            // === 瞳孔 104-105 (2点) ===
            //   104=右瞳孔（与74同位置，MediaPipe 473）
            //   105=左瞳孔（与77同位置，MediaPipe 468）
            473, 468
        )
    ...
    private fun convert468To106(
        ...
        // === 右半边：M0-M16 (389 → 152) - 画面左侧=实际右脸 ===
        setPoint(0, getMpPoint(389))     // 右太阳穴/轮廓起点
        setPoint(1, midPoint(getMpPoint(389), getMpPoint(356)))   // 右鬓角过渡
        setPoint(2, getMpPoint(356))     // 右额侧
        setPoint(3, getMpPoint(454))     // 右耳上
        setPoint(4, midPoint(getMpPoint(454), getMpPoint(323)))   // 右耳上到耳过渡
        setPoint(5, getMpPoint(323))     // 右耳
        setPoint(6, getMpPoint(361))     // 右耳前/脸颊上
        setPoint(7, getMpPoint(288))     // 右脸颊上
        setPoint(8, getMpPoint(365))     // 右脸颊中
        setPoint(9, getMpPoint(397))     // 右脸颊下
        setPoint(10, getMpPoint(379))    // 右下颌角
        setPoint(11, getMpPoint(378))    // 右下巴边缘
        setPoint(12, getMpPoint(400))    // 下巴右侧
        setPoint(13, getMpPoint(377))    // 下巴中右
        setPoint(14, getMpPoint(152))    // 下巴中心
        setPoint(15, midPoint(getMpPoint(152), getMpPoint(148)))  // 下巴中心到尖过渡
        setPoint(16, getMpPoint(148))    // 下巴尖

        // === 左半边：M17-M32 (148 → 162) - 画面右侧=实际左脸 ===
        setPoint(17, getMpPoint(176))    // 下巴中左
        setPoint(18, midPoint(getMpPoint(176), getMpPoint(149)))  // 下巴左过渡
        setPoint(19, getMpPoint(149))    // 下巴左侧
        setPoint(20, getMpPoint(150))    // 左下巴边缘
        setPoint(21, getMpPoint(136))    // 左下颌角
        setPoint(22, getMpPoint(172))    // 左脸颊下
        setPoint(23, getMpPoint(58))     // 左脸颊中
        setPoint(24, getMpPoint(132))    // 左脸颊上
        setPoint(25, midPoint(getMpPoint(132), getMpPoint(93)))   // 左脸颊到耳过渡
        setPoint(26, getMpPoint(93))     // 左耳下/脸颊起点
        setPoint(27, getMpPoint(234))    // 左耳
        setPoint(28, getMpPoint(127))    // 左耳上方
        setPoint(29, midPoint(getMpPoint(127), getMpPoint(162)))  // 左耳上到鬓角过渡
        setPoint(30, getMpPoint(162))    // 左太阳穴
        setPoint(31, midPoint(getMpPoint(162), getMpPoint(389)))  // 左鬓角过渡（循环闭合辅助点）
        setPoint(32, getMpPoint(389))    // 左鬓角/轮廓终点
