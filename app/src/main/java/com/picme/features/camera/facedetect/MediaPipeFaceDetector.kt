...
class MediaPipeFaceDetector(context: Context) {
    ...
    companion object {
        ...
        private val NON_CONTOUR_MAPPING = intArrayOf(
            ...
            // === 鼻梁 44-46 (3点) - 从鼻梁上到鼻尖中心 ===
            // 基于 MediaPipe 鼻子语义关键点（MEDIAPIPE_468_POINTS.md）：
            //   鼻梁中心线：168(眉心/43) → 197(鼻梁上/44) → 5(鼻梁中/45) → 4(鼻尖中心/46)
            //   44=鼻梁上(197), 45=鼻梁中(5), 46=鼻梁下/鼻尖中心(4)
            //   三点沿鼻梁中心线分布，严格居中
            197, 5, 4,

            // === 鼻尖 47-51 (5点) - 从左到右，严格对称 ===
            // 基于 MediaPipe 鼻子语义关键点：
            //   47=左外(98), 48=左内(241), 49=中心(2), 50=右内(461), 51=右外(327)
            // 对称性：47(98)↔51(327)关于中心对称，48(241)↔50(461)关于中心对称
            98, 241, 2, 461, 327,

            // === 右眼 52-57 (6点) - 画面左侧=实际右脸 ===
            // 眼周8点结构：外角 + 上眼睑3点 + 内角 + 下眼睑内角
            // MediaPipe右眼外轮廓（从外角到内角）：226→30→56→133→26→110
            //   52=226(外角), 53=30(上眼睑外), 54=56(上眼睑中), 55=133(上眼睑内)
            //   56=26(内角), 57=110(下眼睑内)
            // 上眼睑3点(53-55)沿外轮廓均匀分布，不含眼角
            226, 30, 56, 133, 26, 110,

            // === 左眼 58-63 (6点) - 画面右侧=实际左脸 ===
            // MediaPipe左眼外轮廓（从外角到内角）：362→286→260→359→339→252
            //   58=362(外角), 59=286(上眼睑外), 60=260(上眼睑中), 61=359(上眼睑内)
            //   62=339(内角), 63=252(下眼睑内)
            // 对称性：59(286)↔61(359)关于眼中心对称
            362, 286, 260, 359, 339, 252,

            // === 右眉下部 64-67 (4点) - 画面左侧=实际右脸，从眉头到眉尾 ===
            // MediaPipe "左眉下部"实际在画面左侧=被摄者右脸，对应火山引擎右眉下部
            //   64=53(眉头下), 65=52(眉腰前下), 66=65(眉峰下), 67=55(眉尾下)
            53, 52, 65, 55,

            // === 左眉下部 68-71 (4点) - 画面右侧=实际左脸，从眉尾到眉头 ===
            // MediaPipe "右眉下部"实际在画面右侧=被摄者左脸，对应火山引擎左眉下部
            //   火山引擎要求从眉尾到眉头：68=285(眉尾下), 69=295(眉峰下), 70=282(眉腰前下), 71=283(眉头下)
            285, 295, 282, 283,

            // === 右眼内/下 72-74 (3点) - 画面左侧 ===
            // 下眼睑3点(不含角)：72=下眼睑中, 73=下眼睑外, 74=瞳孔
            // MediaPipe对应：27=下眼睑中, 23=下眼睑外, 473=右瞳孔
            // 下眼睑内角在57(110)，与72-73组成完整下眼睑3点(不含内角)
            27, 23, 473,

            // === 左眼内/下 75-77 (3点) - 画面右侧 ===
            // 下眼睑3点(不含角)：75=下眼睑中, 76=下眼睑外, 77=瞳孔
            // MediaPipe对应：257=下眼睑中, 253=下眼睑外, 468=左瞳孔
            // 下眼睑内角在63(252)，与75-76组成完整下眼睑3点(不含内角)
            257, 253, 468,
            ...
            //   位于眉心43(168)两侧最近的两个点
            //   193=168左侧最近点(画面左侧), 417=168右侧最近点(画面右侧)
            //   对称性：193↔417 关于168对称
            193, 417,

            // === 鼻孔 80-83 (4点) ===
            // 火山引擎语义：80=右鼻孔左(画面左侧), 81=右鼻孔右, 82=左鼻孔左, 83=左鼻孔右(画面右侧)
            // MediaPipe对应：198=右鼻孔左, 420=右鼻孔右, 49=左鼻孔左, 279=左鼻孔右
            198, 420, 49, 279,
