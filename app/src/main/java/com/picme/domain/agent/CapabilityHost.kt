...
object GlobalCapabilityHost {
    @Volatile
    private var host: CapabilityHost? = null

    fun set(host: CapabilityHost) {
        this.host = host
    }

    fun get(): CapabilityHost? = host
...
fun rememberCapabilityHost(
    ...
    return remember(*capabilities) {
        CapabilityHost(resolvedParent).apply {
...
fun RegisterCapability(
    ...
    host: CapabilityHost = LocalCapabilityHost.current
) {
